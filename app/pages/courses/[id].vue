  ...
  <div class="flex flex-col w-full max-w-full min-w-0">
    <template v-if="!detail">
      <section class="w-full py-4 md:py-5 px-0">
       ...
       <NuxtLink to="/dashboard" class="mt-2 inline-flex text-brand-primary hover:underline">
          Voltar ao Dashboard
        </NuxtLink>
      </section>
    </template>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between gap-4 py-3">
        ...
        <div class="flex items-center gap-1 shrink-0">
         <button type="button"
           ...
           :disabled="!previousLesson" aria-label="Aula anterior" @click="goPrevious">
            <Icon name="heroicons:chevron-left" class="w-5 h-5" />
          </button>
         <button type="button"
            class="p-2 rounded-lg text-text-secondary hover:text-brand-primary hover:bg-brand-surface transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-brand-primary/50"
           :disabled="!nextLesson" aria-label="Próxima aula" @click="goNext">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 lg:gap-8 w-full py-4 lg:min-h-0">
        <div class="flex flex-col gap-6 min-w-0">
         <VideoPlayer :key="currentLessonId || 'no-lesson'"
        ...
        <MaterialsList v-if="currentLesson && currentLesson.materials.length > 0"
            :materials="currentLesson.materials" />
        </div>

      <LessonSidebar :modules="detail.modules" :current-lesson-id="currentLessonId" :completed-count="completedCount"
          :total-lessons="totalLessons" :progress-percent="progressPercent" @select-lesson="selectLesson" />
      </div>
    </template>
  </div>
</template>
